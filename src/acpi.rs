...
use crate::mm::{kmap_identity_mmio, kunmap};
...
impl AcpiHandler for AcpiMapper {
    ...
    unsafe fn map_physical_region<T>(&self, physical_address: usize, size: usize) -> PhysicalMapping<Self, T> {
        ...
        kmap_identity_mmio(start, frame_count as usize).expect("Error identity mapping with ACPI");
    ...
    fn unmap_physical_region<T>(region: &PhysicalMapping<Self, T>) {
        kunmap(VirtAddr::new(region.virtual_start().as_ptr() as u64), region.mapped_length()).unwrap();
    }
}
